...
export function Navbar() {
  ...
  const notesDropdownItems = useMemo(() => {
    ...
    list.forEach((ch: any) => {
      items.push({
        label: ch.name,
        href: ch.navPath || `/notes/${encodeURIComponent(ch.id)}`,
  ...
  useEffect(() => {
    const loadInterviewMeta = async () => {
      try {
        ...
        setInterviewRoles(response.meta?.roles || []);
      } catch (error) {
        console.error("Failed to load interview roles", error);
      }
    };
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                            {interviewRoles.map((role) => (
                              <Link
                                ...
                                className="group relative flex items-center gap-3 p-4 rounded-xl border border-border/50 hover:border-primary/30 bg-card hover:shadow-lg hover:shadow-primary/5 transition-all duration-300"
                              >
                                <div className="w-8 h-8 rounded-lg flex items-center justify-center bg-primary/10 group-hover:scale-110 transition-transform">
                ...
                <div className="px-6 py-10 bg-gradient-to-br from-secondary/50 via-background to-background border-b border-border">
                  <div className="flex items-center gap-4 mt-8">
                    <div className="relative">
                      <div className="absolute -inset-1 bg-gradient-to-r from-primary to-purple-500 rounded-full opacity-20 blur-sm"></div>
                      <img
                        src={user.avatar || "https://e7.pngegg.com/pngimages/84/165/png-clipart-united-states-avatar-organization-information-user-avatar-service-computer-wallpaper-thumbnail.png"}
                        alt={user.name}
                        className="relative w-16 h-16 rounded-full border-2 border-background object-cover shadow-md"
                      />
                      {isAdmin && (
                        <div className="absolute -bottom-1 -right-1 bg-primary text-[10px] font-bold text-white px-2 py-0.5 rounded-full border shadow-sm">
                          ADMIN
                        </div>
                      )}
                    </div>
